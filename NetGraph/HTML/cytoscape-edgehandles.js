...
/***/ (function (module, exports, __webpack_require__) {
    ...
    function existEdge(source_id, target_id) {
        var edges = cy.json().elements.edges;
        for (var i = 0; i < edges.length; i++) {
            var edge_data = edges[i].data;
            var edge_source_id = edge_data.source;
            var edge_target_id = edge_data.target;

            if ((source_id == edge_source_id && target_id == edge_target_id) ||
                (source_id == edge_target_id && target_id == edge_source_id)) {
                return true;
                }
        }
        return false;
            ...
            function addEdgeForDraw(source, target, edgeParams, classes) {
                var sourceType = source.json().data.nodeType.toLowerCase();
                var targetType = target.json().data.nodeType.toLowerCase();

                if (source.json().data.parent == target.json().data.id) return; // Prevent an asset in an asset group creating an edge to its parent
                if (source.json().data.parent != null && (source.json().data.parent == target.json().data.parent)) return; // Prevent an asset in an asset group creating an edge to anpther child in the asset group
                if (
                ...
                if (existEdge(source.id(), target.id()) == true) return;

                var relationship = edge_style == "" ? get_default_edge_relationships(sourceType, targetType) : edge_style;
                var edgeStrengthValue = edge_width;
                var relationshipStrength = "Direct";
                var type = 'Normal';

                return cy.add(getEleJson({
                    group: 'edges',
                    data: {
                        source: source.id(),
                        target: target.id(),
                        ...
                        'labelSize': 10,
                        'color': "rgb(0,0,0)",//default_relationship_strength_color,
                        'enabled': true,
                        'opacity': 1,
                        'impactedValue': 0.0,
                        'edgeStrengthValue': "1.00",
                        'edgeStrengthMinValue': "1.00",
                        'edgeStrengthScore': 1.0,
                        'edgeStrengthDistribution': "['SpecificValue','1.00',null,null,'edge']",
                        'weight': "1",//default_relationship_strength_width,
                        'type': ""
                    }
                }, edgeParams, classes));
            }
            ...
            function makeEdges() {
                ...
                if (!preview) {
                    ...
                    cy.emit('newedge', source, target, previewEles);
                ...
                var source2target = addEdgeForDraw(source, target, this.edgeParams(target), classes);
